// FFmpegCODEC.cpp : Defines the entry point for the console application.
//

#include "stdafx.h"

std::string in_filename  = "";//Input file URL
std::string out_filename  = "";//Input file URL
...
int Parsng_cmd(int argc,_TCHAR* argv[])
{
	...
	{  
		if(strlen(DayBreak::util::UnicodeToAnsi(argv[i])) == 0) //´¦Àí¿Õ×Ö·û´®  
		{  
			tmpPara += char(31);  
		}  
		else  
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if(Parsng_cmd(argc,argv) !=0 || argc < 3)
	{
    ...
    if ((ret = open_input_file(in_filename.c_str())) < 0)
        goto end;
    if ((ret = open_output_file(out_filename.c_str())) < 0)
        goto end;
